...
| Resume storage — inventory mode | Approved repository, submission method, stable document identifier, versions, access and receipt confirmation | Authorized reviewer retrieves the correct version; inventory points to it without embedding a sharing credential |
| Resume storage — custody mode (portal uploads on) | **This work is no longer outside Slate.** Slate stores the documents under `DATA_DIR/application-files/`, so the procedure must instead specify: malware scanning, the privacy and data-use notice, records retention applied to the file directory as well as the store, legal-hold handling, and the accommodation route for anyone who cannot produce a PDF. See `docs/search-operating-procedure.md` §5b | A synthetic applicant's PDF is stored, is openable only by a reviewer on that search and only once a scan has cleared it, appears in a snapshot, and survives a restore with the application that names it |
