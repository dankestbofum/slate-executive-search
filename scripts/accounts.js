...
// consultant session could mint accounts; requiring shell access to
...
const actions = {
  ...
  create([name, email, title]) {
    ...
    const { user } = db.createUser({ name, email, title, role: 'consultant' });
    db.persist();
    console.log('Created ' + user.id + ' (' + user.email + ')');
    console.log('They can sign in with their email.');
  },
...
if (!action || !Object.hasOwn(actions, action)) {
  console.error('Usage: node scripts/accounts.js <list|create|rename|disable|enable> [args]');
