...
${JSON.stringify(room, null, 2)}`
  ...
  questions: s => {
    const keys = require('./questions').keys(s);
    return `Plan all candidate questions together from the adopted profile. Return ONE JSON object with exactly these keys: ${keys.join(', ')}. Each key holds the corresponding complete document below. Do not repeat a question within or across stages, including paraphrases asking for the same example. Initial screening establishes relevant experience; semifinalist questions seek different, deeper evidence; interview questions and scenarios test judgment in new situations. Keep scoring guidance in the interview guide, never in candidate prompts.\n\n${keys.map(key => key + ': ' + SCHEMAS[key]).join('\n\n')}\n\nSearch:\n${JSON.stringify(packSearch(s),null,2)}\n\nExisting question plan to improve:\n${JSON.stringify(Object.fromEntries(keys.map(key => [key,s.artifacts[key] || {}])),null,2)}`;
  },
